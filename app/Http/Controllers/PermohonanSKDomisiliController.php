...
class PermohonanSKDomisiliController extends Controller
{
    ...
    {
        $query = PermohonananSKDomisili::with('masyarakat')->latest();
    ...
    {
        $permohonan = PermohonananSKDomisili::with('masyarakat')->findOrFail($id);
    ...
    {
        $permohonan = PermohonananSKDomisili::findOrFail($id);
        ...
        try {
            ...
            if (is_null($permohonan->nomor_surat)) {
                ...
                $lastNomorUrut = PermohonananSKDomisili::whereYear('tanggal_selesai_proses', $currentYear)->max('nomor_urut') ?? 0;
    ...
    {
        $permohonan = PermohonananSKDomisili::where('id', $id)->where('status', 'selesai')->firstOrFail();
        if ($permohonan->file_hasil_akhir && Storage::disk('public')->exists($permohonan->file_hasil_akhir)) {
            return Storage::disk('public')->download($permohonan->file_hasil_akhir);
        }
