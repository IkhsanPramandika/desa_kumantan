...
class PermohonanSKUsahaController extends Controller
{
    ...
    {
        $query = PermohonananSKUsaha::with('masyarakat')->latest();
    ...
    {
        $permohonan = PermohonananSKUsaha::with('masyarakat')->findOrFail($id);
    ...
    {
        $permohonan = PermohonananSKUsaha::findOrFail($id);
        ...
        try {
            ...
            if (is_null($permohonan->nomor_surat)) {
                ...
                $lastNomorUrut = PermohonananSKUsaha::whereYear('tanggal_selesai_proses', $currentYear)->max('nomor_urut') ?? 0;
    ...
    {
        $permohonan = PermohonananSKUsaha::where('id', $id)->where('status', 'selesai')->firstOrFail();
        if ($permohonan->file_hasil_akhir && Storage::disk('public')->exists($permohonan->file_hasil_akhir)) {
            return Storage::disk('public')->download($permohonan->file_hasil_akhir);
        }
