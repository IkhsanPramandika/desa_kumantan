...
class PermohonanSKTidakMampuController extends Controller
{
    ...
    {
        $query = PermohonananSKTidakMampu::with('masyarakat')->latest();
    ...
    {
        $permohonan = PermohonananSKTidakMampu::with('masyarakat')->findOrFail($id);
    ...
    {
        $permohonan = PermohonananSKTidakMampu::findOrFail($id);
        ...
        try {
            ...
            if (is_null($permohonan->nomor_surat)) {
                ...
                $lastNomorUrut = PermohonananSKTidakMampu::whereYear('tanggal_selesai_proses', $currentYear)->max('nomor_urut') ?? 0;
    ...
    {
        $permohonan = PermohonananSKTidakMampu::where('id', $id)->where('status', 'selesai')->firstOrFail();
        if ($permohonan->file_hasil_akhir && Storage::disk('public')->exists($permohonan->file_hasil_akhir)) {
            return Storage::disk('public')->download($permohonan->file_hasil_akhir);
        }
