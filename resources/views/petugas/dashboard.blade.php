                ...
                <div class="card-body">
                    <div class="row no-gutters align-items-center">
                        <div class="col mr-2">
                            ...
                            <div class="h5 mb-0 font-weight-bold text-gray-800">{{ $overallTotalRevisi ?? 0 }}</div>
                        </div>
                                ...
                                <div class="card border-left-primary shadow-sm h-100">
                                    <div class="card-body">
                                        <div class="d-flex justify-content-between align-items-center">
                                            <div class="mr-3">
                                                <div class="text-primary font-weight-bold text-uppercase mb-1">{{ $details['title'] }}</div>
                                                <div class="small text-gray-600">{{ $stats[$key]['total'] ?? 0 }} Total Permohonan</div>
                                            </div>
                                            @if (($stats[$key]['pending'] ?? 0) > 0)
                                            <span class="badge badge-danger badge-pill" style="font-size: 1rem;">{{ $stats[$key]['pending'] }}</span>
                                            @else
                                            <i class="fas {{ $details['icon'] }} fa-2x text-gray-300"></i>
                                            @endif
                                        </div>
                                    </div>
                                </div>
                    ...
                    <div class="table-responsive">
                        <table class="table table-hover">
                            <tbody>
                                ...
                                <tr>
                                    <td>
                                        <div class="font-weight-bold">{{ $permohonan->getJudulNotifikasi() }}</div>
                                        <small class="text-muted">Diajukan oleh: {{ $permohonan->masyarakat->nama_lengkap ?? 'N/A' }}</small>
                                    </td>
                                    <td class="text-right">
                                        <small>{{ $permohonan->created_at->diffForHumans() }}</small>
                                    </td>
                                    <td class="text-right">
                                        <a href="{{ $permohonan->getRouteTujuan() }}" class="btn btn-sm btn-outline-primary">Lihat</a>
                                    </td>
                                </tr>
                                @empty
                                <tr>
