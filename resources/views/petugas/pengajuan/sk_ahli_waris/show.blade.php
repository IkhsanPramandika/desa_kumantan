...
@section('content')
<h1 class="h3 mb-4 text-gray-800">Detail Permohonan Surat Keterangan Ahli Waris #{{ $permohonan->id }}</h1>

@if (session('success'))<div class="alert alert-success">{{ session('success') }}</div>@endif
@if (session('error'))<div class="alert alert-danger">{{ session('error') }}</div>@endif

<div class="row">
    {{-- KOLOM KIRI: DATA PEWARIS & AHLI WARIS --}}
    <div class="col-lg-7">
        ...
        <div class="card shadow mb-4">
            ...
            <div class="card-body">
                ...
                @elseif($permohonan->status == 'selesai')
                    ...
                    <a href="{{ route('petugas.permohonan-sk-ahli-waris.download-final', $permohonan->id) }}" class="btn btn-success btn-block mb-2"><i class="fas fa-download"></i> Unduh Surat</a>
                ...
                @endif
                <a href="{{ route('petugas.permohonan-sk-ahli-waris.index') }}" class="btn btn-secondary btn-block mt-3"><i class="fas fa-arrow-left"></i> Kembali ke Daftar</a>
            </div>
        </div>
