        ...
        <div class="isi-surat">
            ...
            <table class="tabel-data">
                ...
                @if(!empty($ahliWarisList) && is_array($ahliWarisList))
                    @foreach ($ahliWarisList as $index => $ahli_waris)
                        <tr>
                            <td class="label"><strong>{{ $index + 1 }}. Nama Lengkap</strong></td>
                            <td class="separator">:</td>
                            <td class="data">{{ strtoupper($ahli_waris['nama'] ?? '-') }}</td>
                        </tr>
                         <tr>
            ...
            <table class="tabel-data">
                <tr>
                    ...
                    <td class="separator">:</td>
                    <td class="data">{{ strtoupper($permohonan->nama_pewaris ?? '-') }}</td>
                </tr>
                 <tr>
                    <td class="label">NIK</td>
                    <td class="separator">:</td>
                    <td>{{ $permohonan->nik_pewaris ?? '-' }}</td>
                </tr>
                <tr>
                    <td class="label">Tempat/Tgl Lahir</td>
                    <td class="separator">:</td>
                    <td>{{ $permohonan->tempat_lahir_pewaris ?? '-' }}, {{ $permohonan->tanggal_lahir_pewaris ? \Carbon\Carbon::parse($permohonan->tanggal_lahir_pewaris)->isoFormat('D MMMM YYYY') : '-' }}</td>
                </tr>
                <tr>
                    <td class="label">Tanggal Meninggal</td>
                    <td class="separator">:</td>
                    <td>{{ $permohonan->tanggal_meninggal_pewaris ? \Carbon\Carbon::parse($permohonan->tanggal_meninggal_pewaris)->isoFormat('D MMMM YYYY') : '-' }}</td>
                </tr>
                <tr>
                    <td class="label">Alamat Terakhir</td>
                    <td class="separator">:</td>
                    <td>{{ $permohonan->alamat_pewaris ?? '-' }}</td>
                </tr>
            </table>
