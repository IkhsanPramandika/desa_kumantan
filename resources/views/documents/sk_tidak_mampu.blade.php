...
<body>
    <div class="container">
        ...
        <div class="nomor-surat">
            Nomor : {{ $permohonan->nomor_surat ?? '[NOMOR_SURAT]' }}
        </div>
        ...
        <div class="isi-surat">
            ...
            <table class="tabel-data">
                <tr>
                    ...
                    <td class="data">{{ strtoupper($permohonan->masyarakat->nama_lengkap ?? '-') }}</td>
                </tr>
                <tr>
                    <td class="label">NIK</td>
                    <td class="separator">:</td>
                    <td>{{ $permohonan->masyarakat->nik ?? '-' }}</td>
                </tr>
                <tr>
                    <td class="label">Tempat, Tanggal Lahir</td>
                    <td class="separator">:</td>
                    <td>{{ $permohonan->masyarakat->tempat_lahir ?? '-' }}, {{ $permohonan->masyarakat->tanggal_lahir ? \Carbon\Carbon::parse($permohonan->masyarakat->tanggal_lahir)->isoFormat('D MMMM YYYY') : '-' }}</td>
                </tr>
                <tr>
                    <td class="label">Pekerjaan</td>
                    <td class="separator">:</td>
                    <td>{{ $permohonan->masyarakat->pekerjaan ?? '-' }}</td>
                </tr>
                <tr>
                    <td class="label">Alamat</td>
                    <td class="separator">:</td>
                    <td>{{ $permohonan->masyarakat->alamat_lengkap ?? '-' }}</td>
                </tr>
            </table>
            ...
            <p class="paragraf">Surat keterangan ini dibuat sebagai kelengkapan administrasi untuk keperluan {{ $permohonan->keperluan_surat ?? '[Keperluan]' }} bagi anak/orang tua dari yang bersangkutan, yaitu:</p>
            <table class="tabel-data">
                <tr>
                    <td class="label">Nama</td>
                    <td class="separator">:</td>
                    <td class="data">{{ strtoupper($permohonan->nama_terkait ?? '-') }}</td>
                </tr>
                <tr>
                    <td class="label">NIK</td>
                    <td class="separator">:</td>
                    <td>{{ $permohonan->nik_terkait ?? '-' }}</td>
                </tr>
                <tr>
                    <td class="label">Tempat, Tanggal Lahir</td>
                    <td class="separator">:</td>
                    <td>{{ $permohonan->tempat_lahir_terkait ?? '-' }}, {{ $permohonan->tanggal_lahir_terkait ? \Carbon\Carbon::parse($permohonan->tanggal_lahir_terkait)->isoFormat('D MMMM YYYY') : '-' }}</td>
                </tr>
                 <tr>
                    <td class="label">Jenis Kelamin</td>
                    <td class="separator">:</td>
                    <td>{{ $permohonan->jenis_kelamin_terkait ?? '-' }}</td>
                </tr>
                <tr>
                    <td class="label">Pekerjaan/Sekolah</td>
                    <td class="separator">:</td>
                    <td>{{ $permohonan->pekerjaan_atau_sekolah_terkait ?? '-' }}</td>
                </tr>
            </table>
            @else
            <p class="paragraf">Surat keterangan ini dibuat sebagai kelengkapan administrasi untuk keperluan {{ $permohonan->keperluan_surat ?? '[Keperluan]' }}.</p>
            @endif
        ...
        <div class="tanda-tangan clearfix">
            <div class="blok-ttd">
                ...
                <div class="ttd-area">
                    <img src="{{ public_path('sbadmin/img/ttd_kepala_desa.png') }}" alt="Tanda Tangan Kepala Desa" style="max-height: 80px; max-width: 120px;">
                </div>
